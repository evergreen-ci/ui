...
describe("readBuildVariants", () => {
  it("reads existing buildVariants correctly", () => {
    const currVersions = versions.slice(0, 4);
    ...
    expect(res[0].builds.map((b) => b.id)).toStrictEqual([
      "build_101",
      "build_98",
    ]);
  });
