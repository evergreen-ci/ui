...
describe("SharedSettings / Merge Queue tab feature flag", () => {
  beforeEach(() => {
    vi.resetModules();
  });

  it("hides the Merge Queue nav item when new project navigation is disabled", async () => {
    ...
    expect(screen.queryByDataCy("navitem-merge-queue")).not.toBeInTheDocument();
  });

  it("shows the Merge Queue nav item when new project navigation is enabled", async () => {
    vi.doMock("constants/featureFlags", () => ({
    ...
    expect(screen.getByDataCy("navitem-merge-queue")).toBeInTheDocument();
  });
});
