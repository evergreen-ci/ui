...
describe("editSpawnHostModal", () => {
  ...
  it("renders modal", () => {
    ...
    expect(screen.queryByTestId("edit-spawn-host-modal")).toBeVisible();
  });
  ...
  describe("when default sleep schedule is enabled", () => {
    ...
    it("has all daypicker elements disabled", () => {
      ...
      expect(screen.queryByTestId("edit-spawn-host-modal")).toBeVisible();
      within(screen.getByTestId("daypicker"))
  ...
  describe("when disabling the default sleep schedule", () => {
    it("shows the updated hour count when changing the schedule", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId("hour-options")).toBeVisible();
      });
      await user.click(
        within(screen.getByTestId("hour-options")).getByText("07"),
      );
      await user.click(screen.getByTestId("edit-spawn-host-modal"));
