...
describe("columnHeaders (Variant History)", () => {
  it("renders an initial skeleton for the 7 column headers when loading", () => {
    ...
    expect(screen.queryAllByTestId("loading-header-cell")).toHaveLength(7);
  });
  ...
  it("renders the column headers properly when not loading", async () => {
    ...
    await waitFor(() => {
      expect(screen.queryAllByTestId("loading-header-cell")).toHaveLength(0);
    });
    await waitFor(() => {
      expect(screen.queryAllByTestId("header-cell")).toHaveLength(3);
    });
  });
  ...
  it("should truncate the task name only if it is too long", async () => {
    ...
    await waitFor(() => {
      expect(screen.queryAllByTestId("header-cell")).toHaveLength(2);
    });
  ...
  it("should render the truncated task name for a long column header", async () => {
    ...
    await waitFor(() => {
      expect(screen.queryAllByTestId("header-cell")).toHaveLength(1);
    });
