# This is a script which checks if pnpm codegen was run it used by the check_codegen evergreen task

# path to generated types
types_file="src/gql/generated/types.ts"
BEFORE=$(git status --porcelain | grep "$types_file")
pnpm codegen || {
  echo "pnpm codegen failed to run"
  exit 1
}
...
if [ "$BEFORE" != "$AFTER" ]; then
  ...
  echo "did you forget to run 'pnpm codegen'?"
