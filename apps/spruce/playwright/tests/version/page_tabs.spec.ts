...
const versions = {
  changes: { route: `${versionRoute}/changes`, btn: "changes-tab" },
  tasks: { route: `${versionRoute}/tasks`, btn: "task-tab" },
  duration: { route: `${versionRoute}/task-duration`, btn: "duration-tab" },
};
...
test.describe("page tabs", () => {
  test("Defaults to the task tab and applies default sorts", async ({
    ...
    await expect(page.getByTestId(versions.tasks.btn)).toHaveAttribute(
      "aria-selected",
      "true",
    );
  ...
  test("Applies default sorts on task duration tab", async ({
    ...
    await expect(page.getByTestId(versions.duration.btn)).toHaveAttribute(
      "aria-selected",
      "true",
    );
    await expect(page).toHaveURL(
  ...
  test("Applies default sorts on task tab when switching from another tab without any filters", async ({
    ...
    await expect(page.getByTestId(versions.changes.btn)).toHaveAttribute(
      "aria-selected",
      "true",
    );
  ...
  test("Retains filters even when moving to a tab that isn't a task tab", async ({
    ...
    await page.getByTestId(versions.changes.btn).click();
    await expect(page).toHaveURL(
  ...
  test("should be able to toggle between tabs", async ({
    ...
    await page.getByTestId(versions.tasks.btn).click();
