...
describe("ansiRow", () => {
  ...
  it("should highlight text that match a search term", () => {
    ...
    expect(screen.getByTestId("highlight")).toHaveTextContent("highlight me");
  });
...
`);
  ...
  describe("when rendering lines with priority", () => {
    it("renders a low-priority line in black", () => {
      ...
      expect(screen.getByTestId("ansi-row")).toHaveStyle(`color: ${rgbBlack}`);
    });
    ...
    it("renders a high-priority line in red", () => {
      ...
      expect(screen.getByTestId("ansi-row")).toHaveStyle(`color: ${rgbRed}`);
    });
  });
});
