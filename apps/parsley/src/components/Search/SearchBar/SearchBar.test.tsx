...
describe("searchbar", () => {
  ...
  it("disables properly", () => {
    ...
    expect(screen.getByTestId("searchbar-select")).toHaveAttribute(
      "aria-disabled",
      "true",
    );
  ...
  it("should be able to submit an input by clicking the submit button", async () => {
    ...
    await user.type(input, "test");
    expect(input).toHaveValue("test");
    await user.click(screen.getByTestId("searchbar-submit"));
    expect(onSubmit).toHaveBeenCalledTimes(1);
    expect(onSubmit).toHaveBeenCalledWith("filter", "test");
  });
  it("shows a warning icon if input is invalid", async () => {
    ...
    expect(screen.queryByTestId("searchbar-submit")).toHaveAttribute(
      "aria-disabled",
      "true",
    );
  ...
  it("should be able to change the selected option", async () => {
    ...
    await user.click(screen.getByTestId("searchbar-select"));
    await user.click(screen.getByTestId("highlight-option"));
    await user.type(input, "test");
    expect(input).toHaveValue("test");
    await user.type(input, "{Control>}{enter}");
  ...
  it("should populate input and call onChange when applying a search suggestion", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("search-suggestion-popover")).toBeVisible();
    });
