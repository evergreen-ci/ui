...
describe("buttonRow", () => {
  ...
  describe("copy button", () => {
    it("should be disabled when there are no bookmarks", async () => {
      ...
      expect(screen.getByTestId("copy-text-button")).toHaveAttribute(
        "aria-disabled",
        "true",
      );
      // Tooltip should appear only if button is disabled.
      await user.hover(screen.getByTestId("copy-text-button"));
      await waitFor(() => {
    ...
    it("should indicate when the user has successfully copied to clipboard", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId("copy-text-button")).toHaveTextContent(
          "Copied",
        );
      });
    });
  ...
  describe("download button", () => {
    it("should be disabled when there is no rawLogURL", () => {
      ...
      expect(screen.getByTestId("download-log-button")).toHaveAttribute(
        "aria-disabled",
        "true",
      );
    });
