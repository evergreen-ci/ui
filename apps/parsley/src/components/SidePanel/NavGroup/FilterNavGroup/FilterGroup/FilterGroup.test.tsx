...
describe("filters", () => {
  ...
  it("should be able to toggle editing", async () => {
    ...
    expect(screen.getByTestId("edit-filter-name")).toBeInTheDocument();
    expect(screen.getByTestId("edit-filter-name")).toHaveValue(
      defaultFilter.expression,
    );

    expect(screen.getByTestId("edit-filter-name")).toHaveFocus();
  ...
  it("should call editFilter with the correct parameters", async () => {
    ...
    await user.click(screen.getByLabelText("Edit filter"));
    await user.clear(screen.getByTestId("edit-filter-name"));
    await user.type(screen.getByTestId("edit-filter-name"), "newFilter");
  ...
  it("should prevent the user from submitting an invalid filter", async () => {
    ...
    await user.clear(screen.getByTestId("edit-filter-name"));
    await user.type(
      screen.getByTestId("edit-filter-name"),
      "some [[invalid regex",
    );
    const confirmButton = screen.getByRole("button", {
      name: "Apply",
  ...
  it("should display an error message when the provided filter regular expression is invalid", async () => {
    ...
    await user.hover(screen.getByTestId("validation-error-icon"));
    await expect(
  ...
  it("should reset the component state when the user cancels editing", async () => {
    ...
    expect(screen.getByText(defaultFilter.expression)).toBeInTheDocument();
    expect(screen.queryByTestId("edit-filter-name")).toBeNull();
